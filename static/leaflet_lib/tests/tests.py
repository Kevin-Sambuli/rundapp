...
class AppLoadingTest(SimpleTestCase):
    ...
    def test_init_with_non_default_staticfiles_storage(self):
        ...
        try:
            with self.settings(STATICFILES_STORAGE='leaflet_lib.tests.tests.DummyStaticFilesStorage',
...
class LeafletWidgetRenderingTest(SimpleTestCase):
    ...
    def test_included_media(self):
        ...
        self.assertIn('leaflet_lib/leaflet_lib.js', media_js)
        self.assertIn('leaflet_lib/leaflet_lib.extras.js', media_js)
        self.assertIn('leaflet_lib/leaflet_lib.forms.js', media_js)
        self.assertIn('leaflet_lib/draw/leaflet_lib.draw.js', media_js)

        self.assertIn('leaflet_lib/leaflet_lib.css', media_css)
        self.assertIn('leaflet_lib/draw/leaflet_lib.draw.css', media_css)
...
class LeafletGeoAdminMapTest(LeafletGeoAdminTest):
    ...
    def test_widget_template_overriden(self):
        widget = self.formfield.widget
        output = widget.render('geom', '', {'id': 'geom'})
        self.assertIn(".module .leaflet_lib-draw ul", output)
